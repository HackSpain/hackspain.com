...
Adapt the validated Player Market prototype to HackSpain's existing Astro and React architecture as an opt-in pilot backed by real profiles and offers. Confirmed participants receive a private magic link to review and publish their card. Company representatives verify a work email before sending an offer. Accepted offers appear in the live transfer feed.
...
- The user approved holding the validated standalone experience as the scope baseline and building the complete pilot flow on 2026-08-28.
- Existing infrastructure already provides Neon, Drizzle, Resend and per-signup management links. The pilot reuses those patterns without adding a password or third-party identity provider.
- `92 BUILD` remains presentation-only demo content and is not inferred from private signup answers.

## Implementation Notes

- Default branch: `master`.
- Branch: `txema/player-market-pilot`, following the repository's `person/short-description` convention.
- Durable design: `docs/designs/player-market-pilot.md`.
